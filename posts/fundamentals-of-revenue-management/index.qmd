...
author: "Bazyli Szymanski & Alek Racicot"
date: "2026-02-17"
...
If we knew exactly how many business customers would arrive, the problem would be trivial. Given that the business fare is more expensive, if we know 15 business customers will arrive close to flight departure, we should protect exactly 15 seats for those customers. Similarly, if business customers always booked before leisure customers, we could simply sell the full fare until demand is exhausted, then sell remaining seats at the saver fare. Unfortunately, demand is uncertain and varies from flight to flight—we might expect 15 business customers on average, but the actual number could be anywhere from 0 to 30 or more.

Luckily, Littlewood's Rule [@littlewood1972] can be applied to find how many seats we should protect for our business customers.
...
Try the simulation below to see Littlewood's Rule in action. Adjust the expected business demand (Poisson distributed) and the two fares to watch how the optimal protection level changes.

```{=html}
<iframe src="littlewood_rule_sim.html" width="100%" height="650px" style="border:none;"></iframe>
```

### Key Insights

What you're seeing in the simulation is the foundation of modern revenue management:

1. **Protection increases with demand**: Higher expected business demand means more seats should be protected for the high-fare class.
...
3. **Probability-based decisions**: We're not predicting exactly how many business customers will show up—we're making optimal decisions under uncertainty.

4. **Marginal analysis**: Each seat is evaluated independently based on its expected marginal value.
